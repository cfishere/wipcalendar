	...
	function b($file){		
	    ...
	    //echo "csv path is: " . $this->csvPath;
			...
			foreach($this->CSVrows as $row) {
				...
				if($this->isDateXExpired($row['DueDate'], -5)){
					echo "<br>Skipping job  ".$row['InvcNum']."; Due Date of ".$row['DueDate']." is older than 5 days from today.<br>";
			    ...
			    if( preg_match($patrn,$row['DueDate'],$matches) ){ //find the date	
					...
					//each csv row will be stored in a year array ['2019'], etc, for efficient mapping to 
					//our xml year node(s); Also we need to handle xml year node object is null fatal errors.
					if(!is_array($this->CSVrows[$yr])){
						//establish an array '2019' to hold all jobs due in that yr.
						  ...
						  if( $this->xmlMonthNode == null ){
							 ...
							 //try to skip to the next job of this foreach() CSVrows Year:
							  echo '<br>Skipping '.$csvRow['job'].': There\'s no XML month node for '.
								  $csvRow['month'].'/'.$csvRow['month'].' Need it added today? Copy/email this report to Chris';
						  	 continue;  
						  }
